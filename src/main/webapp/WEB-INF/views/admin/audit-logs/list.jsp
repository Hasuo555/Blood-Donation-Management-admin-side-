...
<div class="card mb-4">
    <div class="card-body">
        <form id="audit-filter-form" method="get" action="<c:url value='/admin/audit-logs'/>" class="filter-form">
            ...
            <div class="form-row form-row--2">
                <div class="form-group">
                    ...
                    <select id="filter-entityType" name="entityType" class="form-control">
                        ...
                        <option value="DONOR"         ${entityType == 'DONOR'         ? 'selected' : ''}>Donor</option>
                        <option value="STAFF"         ${entityType == 'STAFF'         ? 'selected' : ''}>Staff</option>
                        <option value="HOSPITAL"      ${entityType == 'HOSPITAL'      ? 'selected' : ''}>Hospital</option>
                        <option value="ARTICLE"       ${entityType == 'ARTICLE'       ? 'selected' : ''}>Article</option>
                        <option value="BLOOD_REQUEST" ${entityType == 'BLOOD_REQUEST' ? 'selected' : ''}>Blood Request</option>
                        <option value="ACCOUNT"       ${entityType == 'ACCOUNT'       ? 'selected' : ''}>Account</option>
                        <option value="ADMIN"         ${entityType == 'ADMIN'         ? 'selected' : ''}>Admin</option>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="filter-action" name="action" class="form-control">
                        ...
                        <option value="PUBLISH"         ${action == 'PUBLISH'         ? 'selected' : ''}>Publish</option>
                        <option value="ARCHIVE"         ${action == 'ARCHIVE'         ? 'selected' : ''}>Archive</option>
                        <option value="PASSWORD_CHANGE" ${action == 'PASSWORD_CHANGE' ? 'selected' : ''}>Password Change</option>
                    </select>
                </div>
            </div>
            <%-- Row 2: Date Range --%>
            <div class="form-row form-row--2">
                <div class="form-group">
                    ...
                    <input id="filter-startDate" type="datetime-local" name="startDate" class="form-control"
                           value="${fn:escapeXml(startDate)}"/>
                </div>
                <div class="form-group">
                    <label class="form-label" for="filter-endDate">To</label>
                    <input id="filter-endDate" type="datetime-local" name="endDate" class="form-control"
                           value="${fn:escapeXml(endDate)}"/>
                </div>
            </div>
            <%-- Row 3: Performer --%>
            <div class="form-row form-row--1">
                <div class="form-group">
                    <label class="form-label" for="filter-performer">User / Admin Email</label>
                    <input id="filter-performer" type="text" name="performer" class="form-control"
                           placeholder="e.g. admin@amyanhlu.org"
                           value="${fn:escapeXml(performer)}"/>
                </div>
            </div>
            <div class="form-actions">
                <button type="submit" class="btn btn-primary">Filter</button>
                <a href="<c:url value='/admin/audit-logs'/>" class="btn btn-secondary">Reset Filters</a>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="card-body p-0">
        <c:choose>
            <c:when test="${logPage.totalElements == 0}">
                <div class="empty-state"><p>No audit log entries found.</p></div>
            </c:when>
            <c:otherwise>
                <table class="data-table data-table--compact">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>${log.id}</td>
                            <td class="text-sm text-muted text-nowrap">${log.createdAt}</td>
                            <td class="text-sm">${log.account != null ? fn:escapeXml(log.account.email) : '—'}</td>
                            <td><span class="badge badge-action">${fn:escapeXml(log.action)}</span></td>
                ...
                <div class="pagination-bar">
                    ...
                    <span class="pagination-info">Page ${logPage.number + 1} of ${logPage.totalPages}</span>
